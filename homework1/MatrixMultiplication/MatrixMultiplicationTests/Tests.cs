...
public class Tests
{
    ...
    [Test]
    public void ParallelMultiplicationOnSmallMatricies()
    {
        var firstMatrix = new int[,] { { 1 } };
        var secondMatrix = new int[,] { { 1 } };
        var expectedMatrix = new int[,] { { 1 } };
        var resultMatrix = MatrixMultiplication.Matrix.ParallelMatrixMultiplication(firstMatrix, secondMatrix);
        Assert.IsTrue(MatrixMultiplication.Matrix.AreMatricesEqual(expectedMatrix, resultMatrix));

        firstMatrix = new int[,] { { 1 } };
        secondMatrix = new int[,] { { 0, 2 } };
        expectedMatrix = new int[,] { { 0, 2 } };
        resultMatrix = MatrixMultiplication.Matrix.ParallelMatrixMultiplication(firstMatrix, secondMatrix);
        Assert.IsTrue(MatrixMultiplication.Matrix.AreMatricesEqual(expectedMatrix, resultMatrix));
    }
}